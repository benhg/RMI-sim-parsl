...
def XYreplicasim(beta):
    # XY replica Monte Carlo simulation
    global size_global, E_measurements, tau_global, tau_after
    J = 1
    kB = 1  # Boltzmann's constant
    T = 1 / beta  # Temperature constant
...
def vary_temps(betam, betaM, dbeta, savedata='yes'):
    global size_global, todate
    # Build temperature array, data[0]
    if betam == 0:
        betas = arange(betam + dbeta, betaM + dbeta, dbeta)
    else:
        betas = arange(betam, betaM + dbeta, dbeta)

    # Separate core mapping to prevent memory error
    cores = Pool()
    result1 = cores.map(XYmcsim, betas)
    cores.close()
    cores.join()

    cores = Pool()
    result2 = cores.map(XYunionsim, betas)
    cores.close()
    cores.join()

    cores = Pool()
    result3 = cores.map(XYreplicasim, betas)
    cores.close()
    cores.join()
